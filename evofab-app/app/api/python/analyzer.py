...
class ActuatorAnalyzer:
    ...
    """
    def __init__(self, z_min=0.58, z_max=0.70, threshold=200):
        """Configures depth-gating bounds (metres) and the brightness threshold fallback.

        z_min/z_max default reflects the actuator rig's measured working
        distance on a real capture (actuator ~594-644mm from camera) — NOT
        a universal constant. Recalibrate per machine/rig position via
        machine_classification_model's z_min_m/z_max_m columns; these are
        only a reasonable starting point for the current setup."""
        self.z_min = z_min
        self.z_max = z_max
