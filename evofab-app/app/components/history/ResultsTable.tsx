...
export function ResultsTable({ results }: ResultsTableProps) {
  ...
  function exportAllCSV() {
    ...
    const headers = [
      'job_id', 'printer', 'material', 'nozzle_temp', 'cycles', 'pressure_kpa',
      'curvature_before', 'curvature_after', 'delta', 'delta_pct', 'confidence', 'passed', 'created_at',
    ]
  ...
  return (
    <div className="p-5 rounded-xl border border-border bg-surface">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xs font-semibold uppercase tracking-widest text-muted">All Results</h3>
        <button
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border">
              {['#', 'Printer', 'Material', 'Nozzle (°C)', 'Before (mm⁻¹)', 'After (mm⁻¹)', 'Δ', 'Δ %', 'Confidence', 'Timestamp'].map((col) => (
                <th key={col} className="pb-2 pr-4 text-left text-[10px] uppercase tracking-wider text-muted font-medium whitespace-nowrap">
                  {col}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {results.length === 0 ? (
              <tr>
                <td colSpan={10} className="py-8 text-center text-muted">No results yet</td>
              </tr>
            ) : (
              results.map((r, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2.5 pr-4 font-mono text-text">{r.job?.printer?.name ?? '—'}</td>
                    <td className="py-2.5 pr-4 text-muted">{r.job?.material_profile?.name ?? '—'}</td>
                    <td className="py-2.5 pr-4 font-mono text-text">{r.job?.print_settings?.nozzle_temp ?? '—'}</td>
                    <td className="py-2.5 pr-4 font-mono text-blue">{r.curvature_before?.toFixed(4) ?? '—'}</td>
                    <td className="py-2.5 pr-4 font-mono text-teal">{r.curvature_after?.toFixed(4)  ?? '—'}</td>
                    <td className={cn('py-2.5 pr-4 font-mono', isPositive ? 'text-green' : 'text-red')}>
