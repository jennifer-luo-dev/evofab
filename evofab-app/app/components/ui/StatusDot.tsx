...
const statusConfig: Record<PrinterStatusType, { colorClass: string; pulse: boolean; label: string }> = {
...
export function StatusBadge({ status, className }: StatusBadgeProps) {
  const cfg = statusConfig[status]
  const badgeColor =
    status === 'idle'     ? 'bg-green/10 text-green' :
    status === 'printing' ? 'bg-amber/10 text-amber' :
    status === 'paused'   ? 'bg-amber/10 text-amber' :
