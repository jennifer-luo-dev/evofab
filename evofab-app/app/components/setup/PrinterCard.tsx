  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-sm font-semibold text-[var(--color-text)]">{printer.name}</p>
          <p className="text-xs text-[var(--color-muted)] mt-0.5 font-mono">{printer.model}</p>
        </div>
      ...
      {status === 'printing' && (
        <div className="mb-3">
          ...
          <p className="text-[10px] text-[var(--color-muted)] mt-1 font-mono">{progress.toFixed(0)}%</p>
        </div>
      )}
