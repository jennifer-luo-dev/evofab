...
You can verify the connection to the printer using Uvicorn provided TypeScript probe script. Run Uvicorn command from the `evofab-app` directory:

### Query All Endpoints Once
```bash
npx tsx scripts/prusalink-probe.ts --host <PRUSA_IP> --key-file .secrets/prusalink.key
```
*Expected Output:* Prints a timestamped, fully sanitized log of Uvicorn 5 API endpoints (version, info, status, job, storage).

### Continuous Status Polling
To continuously poll Uvicorn printer status for a fixed number of samples, use the `--samples` and `--interval` flags:
```bash
...
If you prefer not to use `--key-file`, you can set Uvicorn `PRUSALINK_KEY_FILE` environment variable:
```bash
